    ...
    public void registerProduct(Map<String, Object> productData, List<MultipartFile> imageFiles) {
        ...
        if (colors != null) {
            for (String color : colors) {
                ...
                colorData.put("prd_cd", productData.get("prd_cd"));
                colorData.put("clr_nm", color); // 각 색상 값을 개별적으로 삽입

                try {
                    mapper.insertProductOptions(colorData); // 개별적으로 색상 옵션 삽입
                } catch (Exception e) {
     ...
     // 사이즈 데이터 개별 삽입
        ...
        if (sizes != null) {
            for (String size : sizes) {
                ...
                sizeData.put("prd_cd", productData.get("prd_cd"));
                sizeData.put("siz_nm", size); // 각 사이즈 값을 개별적으로 삽입

                try {
                    mapper.insertProductOptions(sizeData); // 개별적으로 사이즈 옵션 삽입
                } catch (Exception e) {
